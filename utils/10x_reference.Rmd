---
title: "Introduction to 10x Multiome v2"
...
top10_cells<-names(table(pbmc$celltype))[order(-table(pbmc$celltype))][1:10]
```


```{r}
ref_sub <- subset(pbmc, subset=celltype %in% top10_cells)
```


```{r}
saveRDS(ref_sub, 'pbmc10k.rds')
```
