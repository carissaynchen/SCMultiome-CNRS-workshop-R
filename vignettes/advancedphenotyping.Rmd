...
<span class="fa-stack fa"><i class="fa fa-circle fa-stack-2x"></i><i class="fa fa-pencil-alt fa-stack-1x fa-inverse"></i></span> **Preparation and pre-reading**
...
|------------------------------------|------|
| Introduction                       | 5m  |
| How to infer the a trajectory      | 20m   |
| Visualisation of the trajectory    | 20m   |
| Evaluating the trajectory          | 30m   |
| Characterisation of the trajectory | 60m   |
| Concluding remarks                 |      |
...
p1 <- scater::plotPCA(sce, colour_by = "cell_cluster", point_size=0.5) + ggtitle("PCA")
p2 <- scater::plotTSNE(sce, colour_by = "cell_cluster", point_size=0.5) + ggtitle("TSNE")
p3 <- scater::plotUMAP(sce, colour_by = "cell_cluster", point_size=0.5) + ggtitle("UMAP")
...
scater::plotReducedDim(sce, dimred = "PCA_Cepo", colour_by = "cell_cluster", point_size=0.5) +
  ggplot2::ggtitle("Cepo_PCA")
```
...
```{r fig.height=6, fig.width=12, cache=FALSE}
set.seed(2024)
sce = scater::runTSNE(sce, dimred = "PCA", name="TSNE_Cepo", BPPARAM= BiocParallel::MulticoreParam(workers = 4))

sce = scater::runUMAP(sce, dimred = "PCA", name="UMAP_Cepo", min_dist=0.5,  BPPARAM=BiocParallel::MulticoreParam(workers = 4))

p2 = scater::plotReducedDim(sce, dimred = "TSNE_Cepo", colour_by = "cell_cluster", point_size=0.5) + ggplot2::ggtitle("Cepo_TSNE")
p3 = scater::plotReducedDim(sce, dimred = "UMAP_Cepo", colour_by = "cell_cluster", point_size=0.5) + ggplot2::ggtitle("Cepo_UMAP")
...
dt <- DT::datatable(assoRes, options = list(scrollX=TRUE)) %>%
